...
static `$INSTANCE_NAME`_RS485_address jsonAddressToRS(char atomType, char atomID) {
...
void rs485_selected_as_slave_callback(`$INSTANCE_NAME`_RS485_address byWho) {
	comm.flags.SLAVE = 1;
}
...
void comm_Init(void) {
    ...
    `$INSTANCE_NAME`_RS485_SetCallbackOnSelectedAsSlave(rs485_selected_as_slave_callback);
    `$INSTANCE_NAME`_RS485_SetCallbackOnReleasedAsSlave(rs485_released_as_slave_callback);
    `$INSTANCE_NAME`_RS485_SetCallbackOnSlaveReceive(rs485_receive_callback, comm.recvBuff, RECV_BUFF_LEN);
    `$INSTANCE_NAME`_RS485_SetCallbackOnMasterReceive(rs485_receive_callback, comm.recvBuff, RECV_BUFF_LEN);
    `$INSTANCE_NAME`_RS485_SetCallbackOnWake(rs485_wake_callback);
//    `$INSTANCE_NAME`_RS485_SetCallbackOnError(rs485_error_callback);
    `$INSTANCE_NAME`_RS485_Init();
}
...
void comm_ScheduleMessage(const char *order, int orderLen, char atomType, char atomID) {
	...
	comm.schedAddr = atomType == 'X' ? `$INSTANCE_NAME`_RS485_Protocol_ARB_ADDRESS : jsonAddressToRS(atomType, atomID);
	*msgDestinationHolderAtomType = atomType;
	*msgDestinationHolderAtomId = atomID;

	`$INSTANCE_NAME`_RS485_RequestBus(rs485_bus_granted_callback);
}
...
void rs485_frame_sent_callback(char * buff, uint32 len) {
	if(comm.flags.LAST_MSG) {
		...
		`$INSTANCE_NAME`_RS485_ReleaseSlave(rs485_slave_released_callback);  //if we are slave, this will fail
	} else {
...
void comm_Handler(void) {
	...
	if(comm.flags.RECV) {
		...
		if(validateDestinationAndParse(comm.recvBuff, comm.len) == JSON_MSG_PARSE_OK) {
			if(flags == COMM_FLAG_NACK) {
				...
				if(error_counter++ < 3) {
					//resend message
					`$INSTANCE_NAME`_RS485_SendFrame(respBuff, RESP_BUFF_LEN - respFreeSpace, rs485_frame_sent_callback);
				}
			} else {
					...
					if(retflag != COMM_FLAG_UNKNOWN) {
						...
						*respEnd = '\0';
						...
						if(comm.flags.SLAVE == 0) {
							`$INSTANCE_NAME`_RS485_ReleaseSlave(rs485_slave_released_callback); //we can call this even if we are not master, it'll just fail
						}
					}
	...
	if(comm.flags.SENT) {
		...
		if(timeout_counter > 2) {
			...
			if(comm.flags.SLAVE == 0) {
				`$INSTANCE_NAME`_RS485_ReleaseSlave(rs485_slave_released_callback);
			}
	...
	case COMM_STATE_BUS_GRANTED:
		`$INSTANCE_NAME`_RS485_SelectSlave(comm.schedAddr, rs485_slave_selected_callback);
		break;
	...
	case COMM_STATE_SLAVE_RELEASED:
		`$INSTANCE_NAME`_RS485_ReleaseBus(rs485_bus_released_callback);
		break;
